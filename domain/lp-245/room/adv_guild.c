...
void extra_reset(int arg) {
    ...
    if (arg)
        return;
    move_object("domain/lp-245/obj/book", this_object());
    move_object(clone_object("domain/lp-245/obj/bull_board"), this_object());
    ob = clone_object("domain/lp-245/obj/quest_obj");
    ob->set_name("orc_slayer");
    ob->set_hint("Retrieve the Orc slayer from the evil orc shaman, and give "
                 "it to Leo.\n");
    move_object(ob, "domain/lp-245/room/quest_room");
}
...
ONE_EXIT("domain/lp-245/room/vill_road2", "north", "The adventurers guild",
...
int advance(string arg) {
    ...
    get_level(level);
    if (next_level == 20 && "domain/lp-245/room/quest_room"->count())
        return 1;
    if (level == 0)
        next_exp = exp;
...
int south() {
    ...
    this_player()->move_player("south#domain/lp-245/room/adv_inner");
    return 1;
}
...
int list_quests(string num) {
    ...
    if (num && (sscanf(num, "%d", qnumber) == 1))
        "domain/lp-245/room/quest_room"->list(qnumber);
    else
        "domain/lp-245/room/quest_room"->count();
    return 1;
}
