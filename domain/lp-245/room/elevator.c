...
void heart_beat() {
    ...
    level = dest;
    if (level == 2)
        "domain/lp-245/room/church"->elevator_arrives();
    if (level == 1)
        "domain/lp-245/room/wiz_hall"->elevator_arrives();
}
...
int go_east() {
    ...
    if (level == 2)
        this_player()->move_player("east#domain/lp-245/room/church");
    if (level == 1)
        this_player()->move_player("east#domain/lp-245/room/wiz_hall");
    if (level == 3)
        this_player()->move_player("east#domain/lp-245/room/attic");
    return 1;
}
