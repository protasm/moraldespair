...
void reset(int arg) {
    if (arg)
        return;
    set_light(1);
    dest_dir = ({"domain/lp-245/room/narr_alley", "north"});
...
void init() {
    ::init();
    move_object(clone_object("domain/lp-245/obj/mail_reader"), this_player());
}
...
int query_mail(int silent) {
    ...
    if (!restore_object("domain/lp-245/room/post_dir/" + name) || messages == "")
        return 0;
    if (silent)
        return 1;
    new = "";
