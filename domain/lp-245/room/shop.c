...
TWO_EXIT("domain/lp-245/room/vill_road2", "south", "domain/lp-245/room/storage", "west", "The shop",
...
int do_sell(object ob) {
    ...
    "domain/lp-245/room/store"->store(ob);
    return 1;
}
...
int value(string item) {
    ...
    if (!name_of_item) {
        if ("domain/lp-245/room/store"->value(item))
            return 1;
        write("No such item (");
        write(item);
        write(") here\n");
...
int buy(string item) {
    if (!item)
        return 0;
    "domain/lp-245/room/store"->buy(item);
    return 1;
}
...
int north() {
    ...
    this_player()->move_player("north#domain/lp-245/room/store");
    return 1;
}

int list(string obj) {
    "domain/lp-245/room/store"->inventory(obj);
    return 1;
}
