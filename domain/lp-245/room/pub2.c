...
void reset(int arg) {
    ...
    if (!top_list || !present(top_list, this_object())) {
        top_list = clone_object("domain/lp-245/obj/level_list");
        move_object(top_list, this_object());
    }
    if (!rules || !present(rules, this_object())) {
        rules = clone_object("domain/lp-245/obj/Go/rules");
...
#endif
    this_player()->move_player("west#domain/lp-245/room/yard");
    return 1;
}
...
void start_player() {
    if (!player) {
        ...
        player->set_hp(100);
        player->set_wc(12);
        move_object(player, "domain/lp-245/room/pub2");
        if (!action) {
            action = allocate(2);
            type = allocate(2);
            match = allocate(2);
