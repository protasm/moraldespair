inherit "room/room";

void create() {
  ::create();

  short_desc = "Drifted Platform";
  long_desc = "Scattered piers suggest a once-long span, now broken into bays. Wind has piled sand against one side, leaving the other scoured bare.";
  dest_dir = ({
    "chapter/prologue/area/sunken/room972", "north",
    "chapter/prologue/area/sunken/room427", "south",
    "chapter/prologue/area/sunken/room95", "east",
    "chapter/prologue/area/sunken/room63", "west",
  });

  set_light(1);
}
